...
class GameView(context: Context, attrs: AttributeSet? = null) : SurfaceView(context, attrs), Runnable {
    ...
    private val background: Bitmap = BitmapFactory.decodeResource(resources, R.drawable.background).scale(1100, 2000)
    ...
    private fun update() {
        ...
        while (iterator.hasNext()) {
            ...
            if (obstacle.y - obstacle.height > height) iterator.remove()
        }
        playerX = lanePositions[currentLaneIndex] - playerFrames[0].width / 2
        frameCounter++
    }
    ...
    fun moveDown() { playerY = (playerY + 200f).coerceAtMost(height - 150f) }
    ...
    private fun spawnObstacleRow() {
        ...
        val emptyLanesCount = Random.nextInt(0,2) // max 2 empty lanes
        ...
        lanesArray.forEach { lane ->
            if (!emptyLanes.contains(lane)) {
                ...
                val y = -1000f
                val scale = 0.2f
                obstacles.add(Obstacle(x, y, obstacleBitmap, obstacleBitmap.width, obstacleBitmap.height, scale))
            }
        }
    }
